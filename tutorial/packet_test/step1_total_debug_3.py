...
def assign_module_colors(adata, ggm_key='ggm', seed=0):
    """
    ...
    The palette is automatically selected based on module count:
      - ≤10 modules: use 'tab10'
      - ≤50 modules: use 'tab20'
      - ≤100 modules: use 'nipy_spectral'
      - ≤150 modules: use 'gist_rainbow'
      - ≤200 modules: use 'hsv'
      - ≤300 modules: use 'rainbow'
    
    A random seed is applied for reproducibility.
    
    The generated mapping (module_id -> color hex code) is stored in adata.uns['module_colors'][ggm_key].
    ...
    Returns:
      A dictionary mapping module IDs to color hex codes.
    """
    # 检查 ggm_key 是否在 adata.uns['ggm_keys'] 中
    ...
    mod_col_key = adata.uns['ggm_keys'][ggm_key]['module_colors']
    ...
    # 从 colormap 中均匀采样 n_modules 个颜色
    colors = [mpl.colors.rgb2hex(cmap(i / n_modules)) for i in range(n_modules)]
    colors = np.array(colors)
    # 随机打乱颜色顺序
    perm = np.random.permutation(n_modules)
    colors = colors[perm]
    
    # 构建模块ID到颜色的映射字典
    color_dict = {module_ids[i]: colors[i] for i in range(len(module_ids))}
    
    # 将配色字典存入 adata.uns['module_colors'][ggm_key]
    if mod_col_key not in adata.uns:
        adata.uns[mod_col_key] = color_dict
    else:
        adata.uns[mod_col_key].update(color_dict)
