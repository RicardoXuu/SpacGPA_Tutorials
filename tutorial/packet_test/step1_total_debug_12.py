...
# %%
sc.pl.spatial(adata, alpha_img = 0.5, size = 1.6, title= "annotation_new", frameon = False, color="annotation_new",
              palette=color_dict, show=True)
...
def integrate_annotations_density(
    ...
    alpha, beta, gamma, delta : float
        Weights of the unary terms:
          ...
          delta - low-purity penalty weight (penalizes modules with low global purity)
    lambda_pair : float
    ...
    for t in range(1, max_iter + 1):
        # update module weights based on Leiden purity
        g = nx.Graph()
        ...
        part = leidenalg.find_partition(Graph.from_networkx(g),
