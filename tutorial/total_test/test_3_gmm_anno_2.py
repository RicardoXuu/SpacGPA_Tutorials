...
ggm_gpu_32 = ST_GGM_Pytorch(adata,  round_num=20000, 
                            dataset_name = "E16.5_E1S1", 
                            selected_num=2000, 
                            run_mode=2, 
...
SigEdges_gpu_32 = ggm_gpu_32.SigEdges.iloc[:,0:3]
...
integrate_annotations(adata,
                  ...
                  #module_list=['M11', 'M12', 'M13', 'M14', 'M15', 'M16', 'M17', 'M18', 'M19', 'M20'],
                  #module_list={'M01', 'M02', 'M03', 'M04', 'M05', 'M06', 'M07', 'M08', 'M09', 'M10'},
                  #module_list=adata.uns['module_stats']['module_id'].unique()[adata.uns['module_stats']['module_id'].unique() != 'M09'],
                  #module_list=['M04','M09','M69','M76','M72'],
                  #keep_modules=['M28','M32','M39','M46','M48','M69'],
...
# %%
# 保存注释结果
adata.obs.loc[:,['annotation','annotation_old']].to_csv("data/E16.5_E1S1.MOSTA.annotation.csv")
...
# %%
# 保存可视化结果
sc.pl.spatial(adata, spot_size=1.2, title= "", frameon = False, color="annotation_old", 
              save="/E16_5_E1S1_All_modules_anno_old.pdf",show=False)

# %%
sc.pl.spatial(adata, spot_size=1.2, title= "", frameon = False, color="annotation", 
              save="/E16_5_E1S1_All_modules_anno.pdf",show=False)


# %%
# 保存adata
adata.write("data/MOSTA/E16.5_E1S1.MOSTA_ggm_anno.h5ad")
...
# 1. 原始注释绘图
pdf_file = "figures/MOSTA/All_modules_in_E16_5_E1S1_Module_Anno_Raw.pdf"
...
for module in anno_modules:
    ...
    if len(adata.obs['plot_anno'][adata.obs['plot_anno'] == module]) > 1:
        plt.figure()    
        sc.pl.spatial(adata, spot_size=1.2, title= f"{module}_anno", frameon = False, color="plot_anno",show=False)
        raw_png_file = f"figures/MOSTA/{module}_in_E16_5_E1S1_Module_Anno_Raw.png"
...
# 2. 平滑注释绘图
pdf_file = "figures/MOSTA/All_modules_in_E16_5_E1S1_Module_Anno_Smooth.pdf"
c = canvas.Canvas(pdf_file, pagesize=letter)
image_files = []
for module in anno_modules:
    ...
    if len(adata.obs['plot_anno'][adata.obs['plot_anno'] == module]) > 1:
        plt.figure()
        sc.pl.spatial(adata, spot_size=1.2, title= f"{module}_anno_smooth", frameon = False, color="plot_anno",show=False)
        smooth_png_file = f"figures/MOSTA/{module}_in_E16_5_E1S1_Module_Anno_Smooth.png"
...
# 3. 模块加权表达图
pdf_file = "figures/MOSTA/All_modules_in_E16_5_E1S1_Module_Exp.pdf"
c = canvas.Canvas(pdf_file, pagesize=letter)
image_files = []
for module in anno_modules:
    plt.figure()    
    sc.pl.spatial(adata, spot_size=1.2, title= f"{module}_exp", frameon = False, color=f"{module}_exp", color_map="Reds", show=False)
    raw_png_file = f"figures/MOSTA/{module}_in_E16_5_E1S1_Module_Exp.png"
    plt.savefig(raw_png_file, format="png", dpi=300, bbox_inches="tight")
    plt.close()
    image_files.append(raw_png_file)
