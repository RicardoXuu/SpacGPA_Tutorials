...
# %%
# 使用SpacGPA对 visium_HD Mouse_Small_Intestine 数据进行分析
import numpy as np
import pandas as pd
import random
import time
...
# %%
# 读取空转数据
adata = sc.read_visium("/dta/ypxu/ST_GGM/Raw_Datasets/visium_HD/Mouse_Small_Intestine_FFPE_FFPE/binned_outputs/square_016um/",
...
# 保存GGM
start_time = time.time()
sg.save_ggm(ggm, "data/Mouse_Small_Intestine_FFPE.ggm.h5")
print(f"Time: {time.time() - start_time:.5f} s")

# %%
# 读取GGM
start_time = time.time()
ggm = sg.load_ggm("data/Mouse_Small_Intestine_FFPE.ggm.h5")
print(f"Time: {time.time() - start_time:.5f} s")
...
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="leiden_0.5_ggm", 
              save="/Mouse_Small_Intestine_FFPE_ggm_modules_leiden_0.5.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="leiden_1_ggm",
                save="/Mouse_Small_Intestine_FFPE_ggm_modules_leiden_1.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="louvan_0.5_ggm",
                save="/Mouse_Small_Intestine_FFPE_ggm_modules_louvan_0.5.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="louvan_1_ggm",
                save="/Mouse_Small_Intestine_FFPE_ggm_modules_louvan_1.pdf",show=True)
...
# %%
adata.uns['module_stats'].to_csv("data/Mouse_Small_Intestine_FFPE_ggm_module_stats.csv")
...
# %%
# 保存注释结果
adata.obs.to_csv("data/Mouse_Small_Intestine_FFPE_ggm_annotation.csv")



# %%
# 注释结果可视化并保存可视化结果
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="ggm_annotation", 
              save="/Mouse_Small_Intestine_FFPE_All_modules_annotation.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="ggm_annotation_filtered",
                save="/Mouse_Small_Intestine_FFPE_Filtered_modules_annotation.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="ggm_annotation_no_activity",
                save="/Mouse_Small_Intestine_FFPE_No_activity_modules_annotation.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="ggm_annotation_no_spatial",
                save="/Mouse_Small_Intestine_FFPE_All_modules_annotation_no_spatial.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="ggm_annotation_filtered_no_spatial",
                save="/Mouse_Small_Intestine_FFPE_Filtered_modules_annotation_no_spatial.pdf",show=True)
sc.pl.spatial(adata, size=1.2, alpha_img=0.5, title= "", frameon = False, color="ggm_annotation_no_activity_no_spatial",
                save="/Mouse_Small_Intestine_FFPE_No_activity_modules_annotation_no_spatial.pdf",show=True)


# %%
# 保存adata
adata.write("data/Mouse_Small_Intestine_FFPE_ggm_anno.h5ad")

# %%
# 逐个可视化各个模块的注释结果
anno_modules = adata.uns['module_stats']['module_id']
pdf_file = "figures/visium_HD/Mouse_Small_Intestine_FFPE_all_modules_Anno.pdf"
...
for module in anno_modules:
    ...
    show_png_file = f"figures/visium_HD/Mouse_Small_Intestine_FFPE_{module}_Anno.png"
