...
# %%
# 封包测试3，基本功能完整测试
# 使用 visium_HD - Human Tonsil (Fresh Frozen), Ultima Sequencing -16um 数据
# https://www.10xgenomics.com/datasets/visium-hd-cytassist-gene-expression-libraries-human-tonsil-ff-ultim
import numpy as np
import pandas as pd
import random
import time
...
# df_tissue_positions=pd.read_parquet("data/visium_HD/Human_Tonsil_Ultima/binned_outputs/square_016um/spatial/tissue_positions.parquet")
# df_tissue_positions.to_csv("data/visium_HD/Human_Tonsil_Ultima/binned_outputs/square_016um/spatial/tissue_position.csv", index=False, header=None)
# df_tissue_positions.to_csv("data/visium_HD/Human_Tonsil_Ultima/binned_outputs/square_016um/spatial/tissue_positions_list.csv", index=False, header=None)

# %%
# 读取数据
adata = sc.read_visium("data/visium_HD/Human_Tonsil_Ultima/binned_outputs/square_016um",
...
# %%
# 保存注释结果
adata.obs.loc[:,['annotation','annotation_old']].to_csv("data/Human_Tonsil_Ultima.annotation.csv")
...
sc.pl.spatial(adata, alpha_img = 0.5, size = 1.2, title= "", frameon = False, color="annotation_old", 
              save="/Human_Tonsil_Ultima_All_modules_anno_old.pdf",show=False)

# %%
sc.pl.spatial(adata, alpha_img = 0.5, size = 1.2, title= "", frameon = False, color="annotation", 
              save="/Human_Tonsil_Ultima_All_modules_anno.pdf",show=False)


# %%
# 保存adata
adata.write("data/visium_HD/Human_Tonsil_Ultima_ggm_anno.h5ad")
...
# 1. 原始注释绘图
pdf_file = "figures/visium_HD/All_modules_in_Human_Tonsil_Ultima_Module_Anno_Raw.pdf"
...
for module in anno_modules:
    ...
    if len(adata.obs['plot_anno'][adata.obs['plot_anno'] == module]) > 1:
        ...
        raw_png_file = f"figures/visium_HD/{module}_in_Human_Tonsil_Ultima_Module_Anno_Raw.png"
...
# 2. 平滑注释绘图
pdf_file = "figures/visium_HD/All_modules_in_Human_Tonsil_Ultima_Module_Anno_Smooth.pdf"
c = canvas.Canvas(pdf_file, pagesize=letter)
image_files = []
for module in anno_modules:
    ...
    if len(adata.obs['plot_anno'][adata.obs['plot_anno'] == module]) > 1:
        ...
        smooth_png_file = f"figures/visium_HD/{module}_in_Human_Tonsil_Ultima_Module_Anno_Smooth.png"
...
# 3. 模块加权表达图
pdf_file = "figures/visium_HD/All_modules_in_Human_Tonsil_Ultima_Module_Exp.pdf"
c = canvas.Canvas(pdf_file, pagesize=letter)
image_files = []
for module in anno_modules:
    plt.figure()    
    ...
    raw_png_file = f"figures/visium_HD/{module}_in_Human_Tonsil_Ultima_Module_Exp.png"
    plt.savefig(raw_png_file, format="png", dpi=300, bbox_inches="tight")
    plt.close()
    image_files.append(raw_png_file)
