...
# %%
# 封包测试3，整合各个函数到包内, 高斯混合模型下的细胞聚类相关
...
# %%
adata.obs['M01_exp'].describe()

# %%
adata.X[:1].max()

# %%
sc.pl.spatial(adata, spot_size=1.2, title= "", frameon = False, color="M01_exp", color_map="Reds")
...
def calculate_gmm_annotation(
    adata, 
    modules_list = None,
    max_iter=200,
    prob_threshold=1 - 1e-6,
    min_samples=10,
    n_components=3,
...
# %%
# 测试
calculate_gmm_annotation(adata, 
                         #modules_list=None,
...
# %%
adata.obs['M11_anno'].value_counts()

# %%
adata.uns['module_stats']

# %%
for i in range(len(adata.uns['module_stats'])):
    print(adata.uns['module_stats']['status'][i])



# %%
# 开发函数，对注释结果根据空间坐标或者KNN进行平滑
def smooth_annotations(adata, module_list=None, embedding_key='spatial', k_neighbors=24, min_annotated_neighbors=1):
    ...
    # Save the smoothed annotation matrix to adata.obsm
    adata.obsm['smooth_annotations'] = pd.DataFrame(
        smooth_annotations,
        index=adata.obs_names,
        columns=module_annotations.columns
    )

    print("\nSpatial smoothing completed. Results stored in adata.obs and adata.obsm['smooth_annotations'].\n")
...
# %%
adata.obs['M01_anno_smooth'].value_counts()

# %%
adata.obs['M01_anno'].value_counts()
...
def integrate_annotations_old(adata, module_list=None, result_anno = "annotation", use_smooth=True):
    ...
    adata.obs['annotation'] = cell_annotations
...
# %%
adata.obs['annotation_1'].value_counts()
...
# %%
adata.obs['M76_anno_smooth'].value_counts()


# %%
sc.pl.spatial(adata, spot_size=1.2, title= "", frameon = False, color="annotation_1")
...
# %%
overlap_records[overlap_records['overlap_ratio_union'] > 0.5]

# %%
overlap_records[overlap_records['module_a'] == 'M02']
